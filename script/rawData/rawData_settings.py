...
PPIproteins = PPInetwork.interactorA_id.append(
...
crossref = pd.read_csv( RAWDATA_DIR + "HUMAN_9606_idmapping_selected.tab", "\t",
                        header = None, low_memory = False, names = IDMAP_COLNAMES)

# Export it
crossref.to_csv( RAWDATA_DIR + "HUMAN_9606_idmapping_selected.tab", sep = "\t",
                 index = False, encoding = "utf-8")

# Extract UniProtAC and ENST, and explode ENST into multiple rows
crossref_ENST = explodeValues( crossref, index_col = "UniProtAC",
                               col_to_explode = "Ensembl_TRS", separator = "; ")

# Select only PPI proteins, and sort the df
crossref_ENST = crossref_ENST[ crossref_ENST.UniProtAC.isin( PPInetwork_proteins)
