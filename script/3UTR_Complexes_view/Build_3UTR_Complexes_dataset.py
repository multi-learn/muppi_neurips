...
alter_3UTR = pd.read_csv( RAWDATA_DIR + "RBP-ALL-nascent-complexes_3UTR-alternative.txt",
                          '\t', names = ALTER_3UTR_COL, header = None)
...
alter_3UTR_table = createTable( alter_3UTR, col_to_indexes = "UniProtAC",
...
crossref_ENSG = crossref_ENSG[ (crossref_ENSG.sourceDB == "Ensembl") &
                ( crossref_ENSG.protein_id.isin( PPInetwork_proteins))][[
                "protein_id", "crossReferenceID"]]
...
# Export the table
UTRcomplex_table.to_csv( VIEW_DIR + "3UTR_Complexes.txt", sep = "\t",
...
# Export it
UTRcomplex_table_NoNA.to_csv( VIEW_DIR + "3UTR_Complexes_NoNA.txt", sep = "\t",
                              encoding = "utf-8", index = True)
