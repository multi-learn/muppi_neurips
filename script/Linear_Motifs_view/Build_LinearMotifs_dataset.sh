...
wget "http://elm.eu.org/instances.tsv?q=*" -O instances.tsv

# Select elm instances that have been experimentaly observed in human
# And extract only the list of elm
sed '1,5d' instances.tsv | grep sapiens | grep 'true positive' | cut -f 3 | sort | uniq > elm_instances_TP_HomoSapiens.txt
rm instances.tsv
